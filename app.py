    ...
    if not partners:
        ...
        with bu_col1:
            active_bu = st.radio(
                ...
                horizontal=True,
                key="active_bu",
                help="Select which business unit to validate. DV uses partial (domain-only) matching in addition to full-line matching."
            )
        with bu_col2:
            if active_bu == "DV":
                st.info("🔍 DV Mode: partial domain matching enabled")
        ...
        if not bu_partner_names:
            ...
            def run_validation(doms_to_run, sel_partners, manual_overrides, bu_mode="Demand"):
                ...
                for i, d in enumerate(sorted(doms_to_run)):
                    ...
                    for p in sel_partners:
                        ...
                        if is_dv:
                            # DV mode: classify each line as Full Match / Domain Only / Missing
                            full_matches = []
                            domain_only_matches = []
                            missing_lines = []
                            for l in lines:
                            ...
                            full_pct = round((len(full_matches) / total_lines * 100), 1) if total_lines > 0 else 0.0
                            any_pct = round(((len(full_matches) + len(domain_only_matches)) / total_lines * 100), 1) if total_lines > 0 else 0.0
                            ...
                            if domain_only_matches and show_missing_lines:
                                # Store domain-only as separate key for display
            ...
            if st.button("Validate", type="primary"):
                ...
                if not doms:
                    ...
                    df, missing_detail, crawler_status = run_validation(doms, selected_partners, {}, bu_mode=active_bu)
            ...
            if st.session_state.get("val_results"):
                ...
                if is_dv_results:
                    ...
                    st.caption(
                        "🟢 Full Match = exact line present (ideal) | 🟡 Domain-Only Match = seller domain present but line differs | 🔴 Missing = not found at all"
                    )
                else:
                    styled = (
                        df.style
                ...
                if blocked_doms:
                    ...
                    if st.button(f"Re-validate {len(blocked_doms)} blocked site(s)", type="primary", key="btn_revalidate"):
                        ...
                        if not filled:
                            ...
                            new_df, new_missing, new_status = run_validation(set(filled.keys()), selected_partners, filled, bu_mode=val_bu_mode)
